 ...
 ******************************************************************************
 * @file    main.c
 * @author  taken from @pirohaz
 * @version v1.0
 * @date    23-December-2018
 * @brief   Playing with 2 LEDs and the user push button.
 ******************************************************************************
*/
...
int main(void)
{
	...
	while(1) // Infiinte loop!
	{
		...
		if(ButtonRead){
			// If button is pressed, turn green LED on and blue off
			GPIO_SetBits(LED_GPIO, GreenLED_Pin);
			GPIO_ResetBits(LED_GPIO, BlueLED_Pin);
		}
		else{
			// If button isn't pressed, turn green LED off and blue on
